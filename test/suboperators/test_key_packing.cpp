...
struct KeyPackingTest : public ::testing::TestWithParam<PipelineExecutor::ExecutionMode> {
   KeyPackingTest() : ptr_iu(IR::Pointer::build(IR::Char::build())),
      ...
      exec_pack.emplace(*repiped_pack, mode, "KeyPacking_exec_");
      exec_unpack.emplace(*repiped_unpack, mode, "KeyUnpacking_exec_");
   }
   ...
   void prepareInputChunk() {
      ...
      auto& ptrs_pack = ctx_pack.getColumn(ptr_iu);
      auto& ptrs_unpack = ctx_unpack.getColumn(ptr_iu);
      ...
      for (size_t k = 0; k < 3; ++k) {
         auto& col = ctx_pack.getColumn(src_ius[k]);
         std::generate(col.raw_data, col.raw_data + bytes[k] * num_rows, std::bind(dist, rng));
         ctx_pack.getColumn(src_ius[k]).size = num_rows;
         ctx_unpack.getColumn(out_ius[k]).size = 0;
      }
   };
...
TEST_P(KeyPackingTest, test_pack_unpack) {
   ...
   const auto& ptrs_pack = ctx_pack.getColumn(ptr_iu);
   const auto& ptrs_unpack = ctx_unpack.getColumn(ptr_iu);
   EXPECT_EQ(0, std::memcmp(ptrs_pack.raw_data, ptrs_unpack.raw_data, 1000 * sizeof(char*)));
   for (uint64_t k = 0; k < 3; ++k) {
      const auto& source = ctx_pack.getColumn(src_ius[k]);
      const auto& target = ctx_unpack.getColumn(out_ius[k]);
