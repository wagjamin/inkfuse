...
struct AggregatorSubop : public TemplatedSuboperator<KeyPackingRuntimeState>, public WithRuntimeParams<KeyPackingRuntimeParams> {
   static SuboperatorArc build(const RelAlgOp* source_, const AggState& agg_state, const IU& ptr_iu, const IU& is_init, const IU& agg_iu);
   ...
   protected:
   AggregatorSubop(const RelAlgOp* source_, const AggState& agg_state, const IU& ptr_iu, const IU& is_init, const IU& agg_iu);

   /// Backing aggregate state to be updated.
